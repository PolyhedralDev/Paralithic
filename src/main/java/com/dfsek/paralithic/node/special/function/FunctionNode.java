...
public class FunctionNode implements Optimizable {
    ...
    private final Lazy<Statefulness> statefulness = Lazy.of(
        () -> Statefulness.combine(Statefulness.combine(args.stream().map(Node::statefulness).toArray(Statefulness[]::new)),
            function.statefulness())); // Cache statefulness.
    ...
    public void apply(@NotNull MethodVisitor visitor, String generatedImplementationName) {
        ...
        visitor.visitFieldInsn(GETFIELD, generatedImplementationName, fName,
            "L" + DYNAMIC_FUNCTION_CLASS_NAME + ";"); // Push reference to field to top of stack
        ...
        visitor.visitMethodInsn(INVOKEINTERFACE, DYNAMIC_FUNCTION_CLASS_NAME, "eval", "(L" + CONTEXT_CLASS_NAME + ";[D)D",
            true); // Invoke method
    }
